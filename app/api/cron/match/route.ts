...
async function getAvailableUsers(questionId: string): Promise<Array<{
    ...
    const { answers, error } = await supabase
        .from("daily_answers")
    ...
    const { matches, error: matchError } = await supabase
        .from("matches")
...
export async function GET(request: Request) {
    ...
    const { lock } = await supabase.from("cron_locks").select("locked_at").eq("lock_key", lockKey).maybeSingle();
